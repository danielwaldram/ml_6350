...
def most_common_label(examples, weight_vector):
    dict_of_labels = labels_count(examples, weight_vector)
    highest_count = 0
    highest_label = None
...
# deletes all the examples without the given attribute value
def delete_examples(examples, att_val, attribute, weight_vec):
    new_examples = []
    new_weights = []
    for row in range(len(examples)):
        if examples[row][attribute] == att_val:
            new_examples.append(examples[row])
            new_weights.append(weight_vec[row])
    return new_examples, new_weights


# This is the main ID3 tree building algorithm
def id3(prev_node_examples, node_examples, full_attributes, remain_attributes, weight_vec, prev_node_weight_vec, max_size, gain_type):
    ...
    if len(node_examples) == 0:
        ...
        node['label'] = most_common_label(prev_node_examples, prev_node_weight_vec)
        return node
    ...
    if len(remain_attributes.keys()) == 0:
        node['subnodes'] = 'leaf'
        # they're all the same, so I will make the first examples label the label for the leaf node
        node['label'] = most_common_label(node_examples, weight_vec)
        return node
    ...
    if gain_type == 'entropy':
        best_attribute, best_gain = highest_info_gain_entropy(node_examples, remain_attributes, weight_vec)
    elif gain_type == 'me':
        best_attribute, best_gain = highest_info_gain_me(node_examples, remain_attributes, weight_vec)
    elif gain_type == 'gini':
        best_attribute, best_gain = highest_info_gain_gini(node_examples, remain_attributes, weight_vec)
    else:
    ...
    for attribute_val in full_attributes[best_attribute]:
        ...
        new_node_examples, new_weight_vec = delete_examples(node_examples, attribute_val, best_attribute, weight_vec)
        #PLACEHOLD
        new_prev_node_examples = [x[:] for x in node_examples]
        new_prev_node_weight_vec = weight_vec[:]
        node['subnodes'][attribute_val] = id3(new_prev_node_examples, new_node_examples, full_attributes, new_remain_attributes, new_weight_vec, new_prev_node_weight_vec, max_size, gain_type)
...
def labels_count(examples, weight_vector):
    # Creating a dictionary to hold each label and associated count
    dict_of_labels = {}
    for i in range(len(examples)):
        # If the dictionary item already exists, then it's total is added to by the weight for the given example
        if examples[i][-1] in dict_of_labels:
            dict_of_labels[examples[i][-1]] = dict_of_labels.get(examples[i][-1]) + weight_vector[i]
        # If the dictionary item doesn't already exist, then it's total starts at the weight for the given example
        else:
            dict_of_labels[examples[i][-1]] = weight_vector[i]
...
def entropy_calc(examples, weight_vector):
    ...
    #   for the case of weighted inputs, len(examples) has been replaced with sum(weight_vector) which will
    #   not always be 1 because we aren't always looking at the full set
    entropy = 0
    for key in dict_of_labels.keys():
        entropy = entropy - (float(dict_of_labels[key])/sum(weight_vector))*math.log(float(dict_of_labels[key])/sum(weight_vector), 2)
    return entropy


# Function that calculates the majority error of a given a set of examples. This function only
#   uses the final column of the array, which holds the label
def majority_error_calc(examples, weight_vector):
    # find the most common label
    label_most = most_common_label(examples, weight_vector)
    if label_most is None:
        return 0
    # get counts for every label in dictionary
    label_dict = labels_count(examples, weight_vector)
    # calculate the majority error
    maj_err = (sum(weight_vector) - label_dict[label_most])/float(sum(weight_vector))
    # return
    return maj_err
...
#   uses the final column of the array, which holds the label
def gini_index_calc(examples, weight_vector):
    # get dictionary of labels
    dict_of_labels = labels_count(examples, weight_vector)
    ...
    gi_sum = 0
    for key in dict_of_labels.keys():
        gi_sum = gi_sum + pow(float(dict_of_labels[key]) / sum(weight_vector), 2)
    gi = 1 - gi_sum
    return gi
...
def highest_info_gain_gini(examples, attributes, weight_vector):
    # ME of the set of examples
    set_gi = gini_index_calc(examples, weight_vector)
    ...
    for key in attributes.keys():
        ...
        sub_examples = attribute_split(examples, key, attributes, weight_vector)
        ...
        for row in sub_examples:
            info_gain_sum = info_gain_sum + float(sum(sub_examples[row]["weights"])) / sum(weight_vector) * gini_index_calc(
                sub_examples[row]["examples"], sub_examples[row]["weights"])
...
# Function returns the highest information gain using ME method. Using all the given examples and all the given
#   attributes to determine which attribute splits the best
def highest_info_gain_me(examples, attributes, weight_vector):
    # ME of the set of examples
    set_me = majority_error_calc(examples, weight_vector)
    # dictionary holding all the information gain values
    information_gains = {}
    # highest information gain of the given attributes
    highest_info_gain = 0
    ...
    for key in attributes.keys():
        ...
        # iterating through every row (corresponding to an attribute value) of sub_examples to get contribution to gain
        for row in sub_examples:
            info_gain_sum = info_gain_sum + float(sum(sub_examples[row]["weights"])) / sum(weight_vector) * majority_error_calc(
                sub_examples[row]["examples"], sub_examples[row]["weights"])
        # Calculating the total information gain
...
#   attributes to determine which attribute splits the best
def highest_info_gain_entropy(examples, attributes, weight_vector):
    # entropy of the set of examples
    set_entropy = entropy_calc(examples, weight_vector)
    # dictionary holding all the information gain values
    information_gains = {}
    # highest information gain of the given attributes
    highest_info_gain = 0
    ...
    for key in attributes.keys():
        ...
        # iterating through every row (corresponding to an attribute value) of sub_examples to get contribution to gain
        for row in sub_examples:
            info_gain_sum = info_gain_sum + float(sum(sub_examples[row]["weights"])) / sum(weight_vector) * entropy_calc(
                sub_examples[row]["examples"], sub_examples[row]["weights"])
        # Calculating the total information gain
...
def attribute_split(examples, key, attributes, weight_vec):
    ...
    for k in range(len(attributes[key])):
        ...
        sub_examples[attributes[key][k]]["examples"] = []
        sub_examples[attributes[key][k]]["weights"] = []
        ...
        for i in range(len(temp_examples)):
            ...
            if temp_examples[j][key] == attributes[key][k]:
                sub_examples[attributes[key][k]]["examples"].append(temp_examples[j][-1])
                sub_examples[attributes[key][k]]["weights"].append(temp_weights[j])
                temp_examples.remove(temp_examples[j])
                temp_weights.remove(temp_weights[j])
            else:
                j = j + 1
    return sub_examples
...
def numerical_train_data_preprocessing(examples):
